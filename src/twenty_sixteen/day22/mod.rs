...
impl fmt::Debug for Node {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        if self.used == 0 {
            ...
            // write!(f, "{:02}/{:02}", self.used, self.used+self.available)
            write!(f, "{:02}/{:02}", self.x, self.y)
        }
    }
}

// 29 moves to get it adjacent to

fn is_viable(from: &Node, to: &Node) -> bool {
    (from.x != to.x || from.y != to.y) && from.used > 0 && from.used <= to.available
}

fn get_nodes() -> Vec<Node> {
    let pat: Regex = Regex::new(r"/dev/grid/node-x(?P<x>\d+)-y(?P<y>\d+)\s+(?P<size>\d+)T\s+(?P<used>\d+)T\s+(?P<avail>\d+)T\s+\d+%").unwrap();
    let mut nodes: Vec<_> = INPUT.lines()
        .skip(2)
        .map(|l| {
            let data = pat.captures(l).unwrap();
            Node {
                x: data["x"].parse().unwrap(),
                y: data["y"].parse().unwrap(),
                used: data["used"].parse().unwrap(),
                available: data["avail"].parse().unwrap(),
            }
        })
        .collect();
...
fn part_one() {
    ...
    let mut count_viable = 0;
    let mut total_count = 0;
    for x in 0..width {
        for y in 0..height {
            assert_eq!(x, nodes[x * height + y].x);
            assert_eq!(y, nodes[x * height + y].y);

            for x2 in 0..width {
                for y2 in 0..height {
                    total_count += 1;
                    if is_viable(&nodes[x * height + y], &nodes[x2 * height + y2]) {
                        count_viable += 1;
                    }
                }
            }
    ...
    println!("{}/{} viable", count_viable, total_count);
}

// #[derive(Debug)]
// struct S {x: usize, y: usize, state: Vec<Node2>, cost: usize}


// fn is_viable2(from: &Node2, to: &Node2) -> bool {
//     from.used > 0 && from.used <= to.available
// }

// fn a_star_step(s: S) -> Vec<S> {
//     let width = 32;
//     let height = 28;
//     let mut ret = vec![];
//     for x in 0..width {
...
//     }
//     let mut ret2 = vec![];
//     for (x, y, x2, y2) in ret.into_iter() {
//         let is_payload_move = x == s.x && y == s.y;
//         if is_viable2(&s.state[x * height + y], &s.state[x2 * height + y2])
//                 && (!is_payload_move || s.state[x2 * height + y2].used == 0) {
//             let mut new_grid = s.state.clone();
//             new_grid[x2 * height + y2].used += new_grid[x * height + y].used;
//             new_grid[x2 * height + y2].available -= new_grid[x * height + y].used;
//             new_grid[x * height + y].available += new_grid[x * height + y].used;
//             new_grid[x * height + y].used = 0;
//             ret2.push(S {
//                 x: if is_payload_move {x2} else {s.x},
//                 y: if is_payload_move {y2} else {s.y},
//                 state: new_grid,
//                 cost: s.cost + 1,
//             });
//         }
...
// }
//
// // h(n) = Manhattan distance + total cost so far + linear obstruction
// fn h(s: &S) -> usize {
//     s.cost + s.x * 10 + s.y * 10 + (0..30usize).filter(|x| s.state[x * 28].used > 0).count() * 5
// }

fn part_two() {
    let nodes = get_nodes();
    let (x, y) = nodes.iter().find(|n| n.used == 0).map(|n| (n.x,n.y)).unwrap();
    println!("{},{}", x,y);
    // let nodes2: Vec<_> = nodes.into_iter().map(|n| Node2{used: n.used, available: n.available}).collect();
    // let init_s = S{x:31, y:0, state:nodes2, cost:0};
    // let mut q = vec![init_s];
    // let mut round = 0;
    // loop {
    //     q = q.into_iter().flat_map(a_star_step).collect();
    //     q.sort_by_key(|s| h(s));
    //     if q[0].x == 0 && q[0].y == 0 {
    //         println!("Final cost: {:?}", q[0].cost);
    //         break
    //     }
    //     q.truncate(1000);
    //     round += 1;
    //     if round % 100 == 0 {
    //         let h = h(&q[0]);
    //         let best_cost = q.iter().map(|s| s.cost).min().unwrap();
    //         println!("Round {} done, best so far is {} ({})...", round, h, best_cost);
    //     }
    // }
    let y_max = 28;
    let x_max = 32;
...
fn main() {
    ...
    // 22 up + 16 left + 23 right = 61 to top right
    // Then I need to rotate from the 30th to the 0th position. Each costs 5 (1 down, 2 left, 1 up)
    println!("{}", 22 + 16 + 23 + 30*5);
}

const INPUT: &'static str = "root@ebhq-gridcenter# df -h
