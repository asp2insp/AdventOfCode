...
fn comp<I: U8Input>(i: I) -> SimpleResult<I, Comp> {
    let comparison        = |i, b, r| string(i, b).map(|_| r);
    parse!{i;
        let c =
            comparison(b"<=",  Comp::LTE) <|>
            comparison(b"<", Comp::LT)   <|>
            comparison(b"==",  Comp::EQ)    <|>
            comparison(b"!=", Comp::NEQ) <|>
            comparison(b">=", Comp::GTE) <|>
            comparison(b">", Comp::GT);
        ret c
    }
}
...
fn condition<I: U8Input>(i: I) -> SimpleResult<I, Condition> {
    parse!{i;
        let reg = s() <* token(b' ');
        let comp = comp() <* token(b' ');
        let amount: i64 = signed(decimal);
        ret Condition {
            register: reg,
            comparison: comp,
            amount: amount,
...
fn instruction<I: U8Input>(i: I) -> SimpleResult<I, Instruction> {
    parse!{i;
        let reg = s() <* token(b' ');
        let op = s()  <* token(b' ');
        let amount: i64 = signed(decimal);
...
fn instructions<I: U8Input>(i: I) -> SimpleResult<I, Vec<Instruction>> {
    parse!{i;
        let all = many1(instruction);
        ret all
    }
}

fn part_one(length: usize) {
    let instrs = parse_only(instructions, INPUT.as_bytes()).unwrap();
    assert_eq!(1000, instrs.len());
    let mut registers = HashMap::new();
    let mut max = 0;
    for i in instrs {
        let c_reg = registers.entry(i.condition.register.clone()).or_insert(0).clone();
        let target = i.condition.amount;
        let met = match i.condition.comparison {
            Comp::LT => c_reg < target,
            Comp::LTE => c_reg <= target,
            Comp::EQ => c_reg == target,
            Comp::NEQ => c_reg != target,
            Comp::GT => c_reg > target,
            Comp::GTE => c_reg >= target,
        };
        if met {
            match i.op.as_str() {
                "inc" => {
                    *registers.entry(i.register.clone()).or_insert(0) += i.amount;
                },
                "dec" => {
                    *registers.entry(i.register.clone()).or_insert(0) -= i.amount;
                },
            ...
            max = max.max(*registers.get(&i.register).unwrap());
        }
    }
...
const INPUT: &'static str = "utc dec -736 if p > -7
tn inc -876 if qlm == 4
uz dec 294 if l < 10
a inc -904 if me >= -7
tn inc 622 if ppl <= 6
