...
fn parse_instruction(s: &str) -> Instruction {
    let n: Vec<usize> = s.split_whitespace().map(|si| si.parse().unwrap()).collect();
    Instruction {
        op: n[0],
        a: n[1],
        b: n[2],
...
fn parse_program(s: &str) -> Vec<Instruction> {
    s.lines().skip(3122).map(|l| parse_instruction(l)).collect()
}
...
fn get_working_ops_for_example(e: &Example) -> Vec<Opcode> {
    let table = vec![
        Addr, Addi, Mulr, Muli, Banr, Bani, Borr, Bori, Setr, Seti, Gtri, Gtir, Gtrr, Eqir, Eqri,
        Eqrr,
    ];
    (0..table.len())
        .map(|op| e.with_op(op))
...
fn constraint_collapse(mut possibilities: Vec<HashSet<Opcode>>) -> Option<Vec<Opcode>> {
    ...
    loop {
        ...
        } else if let Some(i) = possibilities
            .iter()
            .enumerate()
            .filter(|(_, p)| p.len() > 1)
...
pub fn part2(input: String) -> String {
    ...
    let eg = parse_examples(&input);
    let possibilities = (0..16)
        .map(|i| {
            eg.iter()
                .filter(|e| e.i.op == i)
                .flat_map(|e| get_working_ops_for_example(e))
                .collect()
        })
        .collect();
