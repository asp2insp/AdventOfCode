...
pub fn part2(input: String) -> String {
	...
	for s in input.split(",") {
		...
		if parts[1].is_empty() {
			...
			bucket[i] = (parts[0].to_string(), parts[1].parse().expect(&format!("Error parsing {:?}", parts)));
		} else {
			// This is a new lens
			bucket.push((parts[0].to_string(), parts[1].parse().expect(&format!("Error parsing {:?}", parts))));
		}
	}
	map.into_iter()
		.enumerate()
		.map(|(i, bucket)| {
			bucket.into_iter().enumerate().map(|(b, (_, l))| (i+1) * (b+1) * l).sum::<usize>()
		})
		.sum::<usize>().to_string()
}
...
#[test]
fn test() {
	assert_eq!(part2("rn=1,cm-,qp=3,cm=2,qp-,pc=4,ot=9,ab=5,pc-,pc=6,ot=7".to_string()), "145".to_string())
}