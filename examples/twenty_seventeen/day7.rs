...
fn parse_line(l: &'static str) -> Program {
    let mut parts = l.split_terminator(" -> ");
    let before = parts.next().expect("?->");
    let above = parts.next()
        .map(|a| a.split_terminator(", ").collect::<Vec<_>>())
        .unwrap_or(Vec::new());
    let name = before.split_whitespace().next().unwrap();
    let weight_s = before.split_whitespace().skip(1).next().unwrap();
    let weight = weight_s[1..weight_s.len()-1].parse::<usize>().unwrap();
    Program {
        name: name,
        weight: weight,
        above: above,
...
fn topo_sort(input: Vec<Program>) -> Vec<Program> {
    let mut result = vec![].into();
    let mut visited = HashSet::new();
    let mut all = HashMap::new();
    for node in input.clone().into_iter() {
        all.insert(node.name, node);
    }
    for node in input.iter() {
        visit(node, &mut result, &mut visited, &all);
    }
    result.into()
}

fn visit(node: &Program, result: &mut VecDeque<Program>, visited: &mut HashSet<&'static str>, all: &HashMap<&'static str, Program>) {
    if visited.contains(node.name) {
        return
    }
    for next in node.above.iter() {
        visit(all.get(next).expect("unvisited"), result, visited, all);
    }
    visited.insert(node.name);
    result.push_front(node.clone());
}

fn part_one() {
    let nodes = INPUT.lines().map(parse_line).collect::<Vec<_>>();
    let sorted = topo_sort(nodes);
    println!("{}", sorted.iter().next().unwrap().name);
}

fn get_weight(root: &'static str, all: &HashMap<&'static str, Program>) -> usize {
    let n = all.get(root).unwrap();
    n.weight + n.above.iter().map(|a| get_weight(a, all)).sum::<usize>()
}
...
fn part_two() {
    ...
    // q.push_back("tknk");
    loop {
        let n = match q.pop_front() {
            Some(n) => n,
            None => break,
        };
        println!("Considering {}", n);
        let above = all.get(n)
            .expect("Gotta be there")
            .above
            .iter()
            .map(|a| (a, get_weight(a, &all)))
            .collect::<HashMap<_,_>>();
        let mut nums: HashMap<usize, usize> = HashMap::new();
        for (_, w) in above.iter() {
            *nums.entry(*w).or_insert(0) += 1;
        }
        if nums.len() <= 1 {
            continue // No discrepancy there
        }
        // Find the odd one out
        println!("{:?}", nums);
        let normal = *nums.iter().max_by_key(|&(_,n)| n).unwrap().0;
        let odd = *nums.iter().min_by_key(|&(_,n)| n).unwrap().0;

        for (a, w) in above.iter() {
            if *w == odd {
                q.push_back(a);
                let diff = (normal as i64) - (odd as i64);
                answer = (all.get(*a).unwrap().weight as i64 + diff) as usize;
            }
        }
    }
    println!("{}", answer)
...
const INPUT: &'static str = "bqyqwn (68) -> wscqe, cwxspl, syogw, xnxudsh
ddswb (34)
hnkvw (320)
