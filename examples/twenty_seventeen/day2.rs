const INPUT: &'static str = "104	240	147	246	123	175	372	71	116	230	260	118	202	270	277	292
...
fn part_one() {
    let sum: i32 = INPUT.lines()
        .map(|l| (
            l.split_whitespace().flat_map(|n| n.parse::<i32>()).max().unwrap(),
            l.split_whitespace().flat_map(|n| n.parse::<i32>()).min().unwrap(),
        ))
        .map(|(m, n)| m - n)
        .sum();
    println!("{}", sum);
...
fn part_two() {
    let sum: i32 = INPUT.lines()
        .map(|l| {
            let nums = l.split_whitespace().flat_map(|n| n.parse::<i32>()).collect::<Vec<_>>();
            for i in 0..nums.len() {
                for j in i..nums.len() {
                    if i == j { continue }
                    let n = nums[i].max(nums[j]);
                    let d = nums[i].min(nums[j]);
                    if n % d == 0 {
                        return n / d
                    }
                }
