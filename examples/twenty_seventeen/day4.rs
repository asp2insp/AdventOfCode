...
fn part_one() {
    let count = INPUT.lines()
        .filter(|p| p.split_whitespace().unique().count() == p.split_whitespace().count())
        .count();
    println!("{}", count);
}

fn get_anagrams(word: &'static str) -> Vec<String> {
    let mut permutations = Vec::new();
    let mut w: Vec<u8> = word.to_owned().into();
    heap_recursive(&mut w[..], |permutation| {
        permutations.push(String::from_utf8(permutation.to_owned()).unwrap())
    });
    permutations.into_iter().unique().collect()
}

fn part_two() {
    let count = INPUT.lines()
        .filter(|p| {
            let a = p.split_whitespace().flat_map(get_anagrams).collect::<Vec<_>>();
            a.iter().unique().count() == a.iter().count()
        })
        .count();
    println!("{}", count);
}
...
const INPUT: &'static str = "oaoe rxeq vssdqtu xrk cjv yaoqp loo
